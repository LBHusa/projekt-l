...
2. **Task reservieren (SOFORT!):**
   ```
   manage_task("update", task_id="...", status="doing", assignee="Worker 2")
   ```
...
## Aktuelle TODO Tasks (Projekt L) - Apple Health Integration

| Priority | Task | Assignee |
|----------|------|----------|
| 100 | Apple Health Backend: DB Migration + API Key Generation | Worker 1 |
| 99 | Apple Health Backend: Webhook Endpoint + Import Logic | Worker 2 |
| 98 | Apple Health Frontend: Integration Settings UI | Worker 3 |
| 97 | Apple Health Frontend: Import History + Sleep Data UI | Worker 4 |

**DEIN TASK (Worker 2):** Webhook Endpoint + Import Logic
- Erstelle `/api/integrations/health-import/webhook/route.ts`
- Erstelle `src/lib/data/health-import.ts`
- Implementiere XP-Berechnung für importierte Daten
- **WICHTIG:** Warte auf Worker 1 (DB Migration muss zuerst fertig sein!)
