...
const FACTION_META: Record<string, { name: string; icon: string }> = {
  karriere: { name: 'Karriere', icon: '💼' },
  hobbys: { name: 'Hobbys', icon: '🎨' },
  koerper: { name: 'Koerper', icon: '💪' },
  geist: { name: 'Geist', icon: '🧠' },
  finanzen: { name: 'Finanzen', icon: '💰' },
  soziales: { name: 'Soziales', icon: '👥' },
  weisheit: { name: 'Weisheit', icon: '📚' },
};
...
async function sendPushNotification(
  ...
  const meta = FACTION_META[faction.faction_id] || { name: faction.faction_id, icon: '📊' };
