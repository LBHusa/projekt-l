...
must_haves:
  truths:
    - "User attempting to create a Quest with <script>alert(1)</script> in title sees validation error"
    - "User attempting to create a Habit with malicious HTML in description sees validation error"
    - "User updating profile with XSS payload in bio receives sanitized output"
  artifacts:
    - path: "package.json"
